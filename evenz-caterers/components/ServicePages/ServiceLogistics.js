      ...
      <div className="min-h-screen bg-gray-50 text-gray-700 ">
         <div className="max-w-5xl mx-auto space-y-5">
            ...
            <div className="bg-indigo-600 text-white rounded-lg shadow-sm p-4 sm:p-6">
               <h1 className="text-xl  sm:text-2xl font-bold ">Services & Logistics</h1>
               <p className="mt-1 text-sm sm:text-base">Configure your catering services, staff details, and logistics information</p>
            </div>
            ...
            <SectionCard
               title="Water Service"
               sectionName="waterService"
               onSave={() => saveSection('waterService', services.waterService)}
               saving={saving}
               errors={errors}
               expandedSections={expandedSections}
               editingSections={editingSections}
               ...
               toggleEditing={toggleEditing}
               showEditInCollapsed={true}
               summary={`${services.waterService.includedInPackage ? 'Included in package' : 'Not included'} - Jar water: ₹${services.waterService.jarWaterCharges}, Bottle water: ₹${services.waterService.bottleWaterCharges}`}
            >
               <div className="space-y-4">
                  <div>
                     <label className="flex items-center space-x-2 cursor-pointer">
                        <input
                           type="checkbox"
                           checked={services.waterService.includedInPackage}
                           onChange={(e) => updateServices('waterService.includedInPackage', e.target.checked)}
                           disabled={!editingSections.waterService}
                           className="rounded border-gray-300 text-blue-600 focus:ring-blue-500 focus:ring-offset-0 disabled:text-gray-700 disabled:opacity-100"
                        />
                        <span className="text-sm text-gray-700 select-none">Water service included in package</span>
                     </label>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                     <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Jar Water Charges</label>
                        <div className="relative">
                           <span className="absolute left-3 top-2 text-gray-500">₹</span>
                           <input
                              type="number"
                              min="0"
                              value={services.waterService.jarWaterCharges}
                              onChange={(e) => updateServices('waterService.jarWaterCharges', Number(e.target.value) || 0)}
                              disabled={!editingSections.waterService}
                              className="w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors disabled:bg-gray-100 disabled:opacity-50"
                           />
                        </div>
                     ...
                     <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Bottle Water Charges</label>
                        <div className="relative">
                           <span className="absolute left-3 top-2 text-gray-500">₹</span>
                           <input
                              type="number"
                              min="0"
                              value={services.waterService.bottleWaterCharges}
                              onChange={(e) => updateServices('waterService.bottleWaterCharges', Number(e.target.value) || 0)}
                              disabled={!editingSections.waterService}
                              className="w-full pl-8 pr-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors disabled:bg-gray-100 disabled:opacity-50"
                           />
                        </div>
...
export default ServicesLogistics;